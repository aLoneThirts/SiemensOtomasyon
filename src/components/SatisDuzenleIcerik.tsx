...
const SatisDuzenleIcerik: React.FC<Props> = ({ subeKodu, satisId, drawerMode = false, onKaydet, onIptal }) => {
  ...
  const handleGeri = () => { if (onIptal) onIptal(); else navigate('/dashboard'); };
  ...
  const fetchSatisDetay = async () => {
    try {
      ...
      if (!sube) return;
      const satisDoc = await getDoc(doc(db, `subeler/${sube.dbPath}/satislar`, satisId));
      if (satisDoc.exists()) {
  ...
  useEffect(() => {
    if (satisId) { fetchSatisDetay(); urunCacheYukle(); kampanyalariCek(); yesilEtiketleriCek(); kesintiCacheYukle(); }
  }, [satisId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try{
      ...
      if(!isAdmin){
        const fresh=await getDoc(doc(db,`subeler/${sube.dbPath}/satislar`,satisId));
        if(fresh.exists()&&fresh.data().onayDurumu===true&&fresh.data().satisDurumu!=='IPTAL'){alert('❌ Bu satış az önce admin tarafından onaylandı.');handleGeri();return;}
      }
      ...
      const oldSnap=await getDoc(doc(db,`subeler/${sube.dbPath}/satislar`,satisId));
      ...
      try{const newSnap=await getDoc(doc(db,`subeler/${sube.dbPath}/satislar`,satisId));const newData=newSnap.exists()?newSnap.data():{};await writeSatisAuditLog({saleId:satisId,satisKodu:satis.satisKodu||'',dbPath:sube.dbPath,branchId:subeKodu||'',branchName:sube.ad,oldData,newData,userId:currentUser?.uid||'',userName:`${currentUser?.ad||''} ${currentUser?.soyad||''}`.trim()});}catch(logErr){console.error('Audit log yazılamadı:',logErr);}
      alert('✅ Satış başarıyla güncellendi!');
      if(onKaydet)onKaydet();else navigate('/dashboard');
          ...
          <div className="duzenle-popup">
            ...
            <div className="duzenle-popup-statu-sec">
              ...
              <div className="duzenle-popup-statu-secenekler">
                <label className={`duzenle-popup-statu-btn ${iptaldenCikarStatusu==='BEKLEMEDE'?'secili':''}`}><input type="radio" name={`statu-${satisId}`} value="BEKLEMEDE" checked={iptaldenCikarStatusu==='BEKLEMEDE'} onChange={()=>setIptaldenCikarStatusu('BEKLEMEDE')} />⏳ Beklemede</label>
                <label className={`duzenle-popup-statu-btn ${iptaldenCikarStatusu==='ONAYLI'?'secili':''}`}><input type="radio" name={`statu-${satisId}`} value="ONAYLI" checked={iptaldenCikarStatusu==='ONAYLI'} onChange={()=>setIptaldenCikarStatusu('ONAYLI')} />✅ Onaylı</label>
              </div>
            </div>
            <div className="duzenle-popup-butonlar">
      ...
      <form onSubmit={handleSubmit} className="duzenle-form">
        ...
        <div className="duzenle-section">
          ...
          <div className="duzenle-urun-header">
            ...
            <span style={{textAlign:'center'}}>Teslim</span><span></span>
          </div>
        ...
        <div className="duzenle-section">
          ...
          <div className="duzenle-odeme-blok">
            ...
            {pesinatlar.map(p=>(<div key={p.id} style={{display:'flex',gap:8,marginBottom:8}}><input type="number" min="0" placeholder="Tutar" value={p.tutar||''} onChange={e=>handlePesinatChange(p.id,'tutar',e.target.value)} className="duzenle-input mono" style={{flex:1}} disabled={alanlarKilitli} /><input type="text" placeholder="Açıklama" value={p.aciklama} onChange={e=>handlePesinatChange(p.id,'aciklama',e.target.value)} className="duzenle-input" style={{flex:2}} disabled={alanlarKilitli} />{!alanlarKilitli&&<button type="button" onClick={()=>pesinatSil(p.id)} className="duzenle-btn-remove">Sil</button>}</div>))}
            ...
            <div style={{gridColumn:'1 / -1'}}>
              ...
              <div style={{display:'flex',gap:12,alignItems:'center',marginTop:4}}>
                <label style={{display:'flex',alignItems:'center',gap:8,cursor:alanlarKilitli?'not-allowed':'pointer',opacity:alanlarKilitli?0.6:1}}>
                  <input type="radio" name={`teslim-${satisId}`} value="evet" checked={teslimEdildiMi===true} onChange={()=>!alanlarKilitli&&setTeslimEdildiMi(true)} disabled={alanlarKilitli} />
                ...
                <label style={{display:'flex',alignItems:'center',gap:8,cursor:alanlarKilitli?'not-allowed':'pointer',opacity:alanlarKilitli?0.6:1}}>
                  <input type="radio" name={`teslim-${satisId}`} value="hayir" checked={teslimEdildiMi===false} onChange={()=>!alanlarKilitli&&setTeslimEdildiMi(false)} disabled={alanlarKilitli} />
        ...
        <div className="duzenle-actions">
          <button type="button" onClick={handleGeri} className="duzenle-btn-cancel">{drawerMode?'← Geri':'İptal'}</button>
...
export default SatisDuzenleIcerik;